...
public final class PasswordDAO
	...
	public UserAccessControl storeNewPassword(final Password thePassword, final User creator )
        ...
        return UserPasswordAccessControlDAO.getInstance().create(creator, thePassword, PasswordPermission.MODIFY);
	}
    ...
    public Password create(final User theCreator, final Group adminGroup,
        ...
        UserAccessControl newUac =
        		UserPasswordAccessControlDAO.getInstance().create(theCreator, newPassword, PasswordPermission.MODIFY, false);

        write(newPassword, newUac);

        UserPasswordAccessControlDAO.getInstance().write(theCreator, newUac);
        ...
        if( adminGroup != null ) {
            GroupPasswordAccessControlDAO.getInstance().create(adminGroup, newPassword, PasswordPermission.MODIFY);
    ...
    public void write(final Password password, final Group group)
            throws SQLException, GeneralSecurityException, IOException {
    	GroupAccessControl gac = GroupPasswordAccessControlDAO.getInstance().create(group, password, PasswordPermission.MODIFY);
        GroupPasswordAccessControlDAO.getInstance().write(group, gac);
	...
	private void performRawAPIUserSearch(final User user, final String searchUsername,
	    ...
	    for(Password password: getMultiple(SEARCH_ALL_PASSWORDS_FOR_LOCATIONS_BY_USER_SQL, user.getId(), searchLocation)) {
            addIdIfMatches(ids, password, UserPasswordAccessControlDAO.getInstance().get(user, password.getId()), searchUsername);
        }
	}
	...
	private void performRawAPIGroupSearch(final User user, final String searchUsername,
        ...
        for(Password password: getMultiple(SEARCH_ALL_PASSWORDS_FOR_LOCATIONS_BY_GROUP_SQL, user.getId(), searchLocation)) {
            addIdIfMatches(ids, password, GroupPasswordAccessControlDAO.getInstance().get(user, password.getId()), searchUsername);
        }
	}
